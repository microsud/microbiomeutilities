...
if(file.exists("AlphaDiversity")) {
  message("AlphaDiversity folder already exists will data will be overwritten")
} else{
...
if(file.exists("BetaDiversity")) {
  message("BetaDiversity folder already exists will data will be overwritten")
} else{
...
if(file.exists("Others")) {
  message("Others folder already exists will data will be overwritten")
} else{
...
if(file.exists("PhyloseqObjects")) {
  message("PhyloseqObjects folder already exists will data will be overwritten")
} else{
...
The counts are log10(1+x) tranformed and then used for ordinations.

```{r BC-PCoA, fig.height=4, fig.width=8}

ps3.log <- microbiome::transform(ps3, "log10")

bc.pcoa <- ordinate(ps3.log, method = "PCoA", distance = "bray")

bc.pcoa.plot <- plot_ordination(ps3.log, bc.pcoa, 
                                type = "split", axes = 1:2,
...
ps3_bray <- phyloseq::distance(ps3.log, method = "bray")
...
wunifrac.pcoa <- ordinate(ps3.log, method = "PCoA", distance = "wunifrac")

wunifrac.pcoa.plot <- plot_ordination(ps3.log, wunifrac.pcoa, 
                                type = "split", axes = 1:2,
                                color = VariableA, shape = VariableB, 
                                label = NULL, 
...
# Calculate bray curtis distance matrix
ps3_wunifrac <- phyloseq::distance(ps3.log, method = "wunifrac")
...
unifrac.pcoa <- ordinate(ps3.log, method = "PCoA", distance = "unifrac")

unifrac.pcoa.plot <- plot_ordination(ps3.log, unifrac.pcoa, 
                                type = "split", axes = 1:2,
                                color = VariableA, shape = VariableB, 
                                label = NULL, 
...
if (heatmap == TRUE) {
  ...
  pseq.famlog10 <- transform(pseq.fam, "log10")
  ps3.log.heatmap <- plot_composition(pseq.famlog10, 
  ...
  ps3.log.heatmap <- ps3.log.heatmap + theme(legend.position = "bottom") + 
    theme_bw() + theme(axis.text.x = element_text(angle = 90)) + 
    ggtitle("Heatmap Log10(1+x) tranformed") + 
    theme(axis.text = element_text(face = "italic"))
  
  ps3.log.heatmap <- ps3.log.heatmap + 
    scale_fill_distiller(palette = "YlOrRd", trans = "reverse")

  print(ps3.log.heatmap)
  
}

ggsave("./Others/Heatmap Log10(1+x) tranformed Family.pdf", 
       plot = ps3.log.heatmap,
       height = 6,
       width = 10)
